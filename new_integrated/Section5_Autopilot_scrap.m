...
%   1. Disturbance is now a PERMANENT step (was 0.5s pulse)
%   2. All figures have FontSize>=12, axis labels, legends, titles
%   3. Commanded values shown as dashed reference lines on every plot
%   4. Control input saturation limits marked on every input plot
%
...
dlim = 40*pi/180;   % 40 deg actuator limit
...
% --- Roll (Section 6.3.1) ---
%wn_phi   = 3;
%kp_phi   = -wn_phi^2 / abs(a_phi2);
kp_phi = (delta_max_a / e_max_phi) * sign(a_phi2);
wn_phi = sqrt(abs(a_phi2) * abs(kp_phi));
zeta_phi = 0.707;
kd_phi   = (2*zeta_phi*wn_phi - a_phi1) / a_phi2;
ki_phi   = 0;

% fprintf('SIGN CHECK: kp_phi=%.4f (need <0), kd_phi=%.4f (need >0)\n', kp_phi, kd_phi);
% if kp_phi > 0, error('kp_phi is POSITIVE — roll will diverge!'); end
% if kd_phi < 0, error('kd_phi is NEGATIVE — roll will diverge!'); end
fprintf('Roll gains: kp_phi=%.4f  kd_phi=%.4f\n', kp_phi, kd_phi);
fprintf('  a_phi1=%.3f (natural damping)  2*zeta*wn=%.3f\n', a_phi1, 2*zeta_phi*wn_phi);
% Note: kd_phi can be negative at high Va when natural damping > desired damping.
% The pole-placement formula still places poles correctly regardless of kd sign.
if kp_phi > 0
    error('kp_phi is POSITIVE — check a_phi2 sign in compute_transfer_functions');
end
if kd_phi < 0
    fprintf('  WARNING: kd_phi<0 — natural damping exceeds desired; pole placement still valid.\n');
end
...
% --- Sideslip (P-only, Section 6.3.3) ---
%kp_beta = 0.5;
%ki_beta = 0;
kp_beta = (delta_max_r / e_max_beta) * sign(a_beta2);
...
kp_theta = (delta_max_e / e_max_theta) * sign(a_theta3);
...
fprintf('\nBandwidth summary:\n');
fprintf('  Roll:        wn=%.2f rad/s\n', wn_phi);
fprintf('  Course:      wn=%.3f rad/s  (sep=%.0f)\n', wn_chi, wn_phi/wn_chi);
fprintf('  Pitch:       wn=%.2f rad/s\n', wn_theta);
fprintf('  Altitude:    wn=%.3f rad/s  (sep=%.0f)\n', wn_h, wn_theta/wn_h);
fprintf('  Va/throttle: wn=%.2f rad/s\n', wn_Vdt);
fprintf('  Va/pitch:    wn=%.2f rad/s\n\n', wn_Vth);
...
phi_cmd   = 20*pi/180;    % 20 deg roll step
chi_cmd   = 30*pi/180;    % 30 deg course step
theta_cmd =  5*pi/180;    % 5 deg pitch step
h_cmd     = 3500;         % ft  altitude step
Va_cmd    = 860;          % ft/s airspeed step

dist_mag  = 0.2;          % permanent step disturbance magnitude
dist_t    = 8;            % time at which disturbance is applied [s]
...
function [t,X,U] = sim_roll(x0,u0,phi_cmd,dm,dt_d,kp,kd,ki,kpb,~,dlim,T,dt,P)
% KEY FIX: disturbance is a PERMANENT step (was a 0.5s pulse)
    t=(0:dt:T)'; N=length(t);
    X=zeros(N,12); U=zeros(N,4); X(1,:)=x0';
    ph_int=0; wnd=zeros(6,1);
    oo=odeset('RelTol',1e-6,'AbsTol',1e-8);
    for k=1:N-1
        x=X(k,:)'; phi=x(7); p=x(10);
        dist = dm * (t(k) >= dt_d);   % PERMANENT step — no upper time limit
        e    = phi_cmd - phi;
...
function [t,X,U] = sim_course(x0,u0,chi_c,kp_ph,kd_ph,ki_ph,kp_ch,ki_ch,kpb,~,dlim,T,dt,P)
    t=(0:dt:T)'; N=length(t);
    X=zeros(N,12); U=zeros(N,4); X(1,:)=x0';
    ch_int=0; ph_int=0; phi_mx=35*pi/180; wnd=zeros(6,1);
    oo=odeset('RelTol',1e-6,'AbsTol',1e-8);
    for k=1:N-1
...
function [t,X,U] = sim_Va_throttle(x0,u0,Va_c,kpV,kiV,dlim,T,dt,P)
    ...
    for k=1:N-1
        x=X(k,:)';
        Va   = forces_moments(x,u0,wnd,P); Va=Va(7);
...
function [t,X,U] = sim_Va_pitch(x0,u0,Va_c,kpV,kiV,kp_th,kd_th,ki_th,dlim,T,dt,P)
    ...
    for k=1:N-1
        ...
        Va  = forces_moments(x,u0,wnd,P); Va=Va(7);
        eV  = Va_c - Va; V_int=V_int+eV*dt;
        th_c= sat(-(kpV*eV+kiV*V_int), thmax);    % pitch down = speed up
        e_t = th_c - x(8); th_int=th_int+e_t*dt;
        de  = sat(u0(1)+kp_th*e_t-kd_th*x(11)+ki_th*th_int, dlim);
        d   = [de;u0(2);u0(3);u0(4)]; U(k,:)=d';
        [~,Xk] = ode45(@(t,x)eom(x,d,wnd,P),[0,dt],x,oo);
