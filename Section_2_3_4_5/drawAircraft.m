...
% --- Horizontal tail ---
% Root x anchored at fuselage mid-ring x = -fuse_len_aft*0.5
% so tails are flush with fuselage surface (no gap)
ht_root_x  = -fuse_len_aft*0.5;   % = -16.5 ft  (fuselage mid-ring)
ht_te_x    = -fuse_len_aft;        % = -33.0 ft  (fuselage tail tip)
ht_span    =  11*sc;               % semi-span to tip
ht_tip_le_x=  ht_te_x + 3*sc;     % tip leading edge
ht_tip_te_x=  ht_te_x;            % tip trailing edge flush with tail tip
ht_dihedral= -1.0*sc;             % slight anhedral

% Root y/z: sit ON the fuselage surface at mid-ring
ht_root_y  =  fuse_w*0.8;         % = 3.2 ft (matches fuselage mid-ring width)
ht_root_z  =  0;                   % at fuselage centreline height

V_htail_r = [...
   ht_root_x,   ht_root_y,   ht_root_z;        % 1 root leading edge (on fuse)
   ht_te_x,     ht_root_y*0.3, ht_root_z;      % 2 root trailing edge (tapered fuse)
   ht_tip_te_x, ht_span,     ht_dihedral;       % 3 tip trailing edge
   ht_tip_le_x, ht_span,     ht_dihedral;       % 4 tip leading edge
];
V_htail_l = [...
   ht_root_x,   -ht_root_y,   ht_root_z;
   ht_te_x,     -ht_root_y*0.3, ht_root_z;
   ht_tip_te_x, -ht_span,     ht_dihedral;
   ht_tip_le_x, -ht_span,     ht_dihedral;
];

% --- Vertical tail ---
% Root also anchored at fuselage mid-ring, top surface (z = -fuse_h*0.8)
vt_root_x  = -fuse_len_aft*0.5;   % = -16.5 ft (fuselage mid-ring)
vt_te_x    = -fuse_len_aft;        % = -33.0 ft (fuselage tail tip)
vt_tip_x   =  vt_te_x + 4*sc;     % tip leading edge x
vt_height  =  10*sc;               % fin height upward
vt_root_z  = -fuse_h*0.8;         % root sits on TOP of fuselage mid-ring

V_vtail = [...
   vt_root_x, 0,  vt_root_z;                    % 1 root leading edge (on fuse top)
   vt_te_x,   0,  vt_root_z*0.3;                % 2 root trailing edge (tapers with fuse)
   vt_te_x,   0,  vt_root_z - vt_height*0.6;    % 3 trailing edge mid
   vt_tip_x,  0,  vt_root_z - vt_height;        % 4 tip
   vt_root_x, 0,  vt_root_z - vt_height*0.5;    % 5 leading edge mid
];
...
% -------------------------------------------------------
% FIX: mode check is now separate from nargin check
% -------------------------------------------------------
if mode == 1
    % CORRECT: rotate about CG first, then translate
    V_rotated    = (R_bv * V_body')';
    V_world(:,1) = V_rotated(:,1) + pn;
    V_world(:,2) = V_rotated(:,2) + pe;
    V_world(:,3) = -V_rotated(:,3) - pd;    % flip z for altitude-up display
else
    % WRONG (Part iv): translate first, then rotate about world origin
    V_trans      = V_body;
